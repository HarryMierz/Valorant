...
def get_match_id(conn, event, match_date, team_one_name, team_two_name):
    query = f'SELECT match_id FROM match_statistics.match WHERE event = \'{event}\' AND match_date = \'{match_date}\' AND team_one_name = \'{team_one_name}\' AND team_two_name = \'{team_two_name}\';'
...
def insert_match_team(conn, match_team_df, match_id):
    for row in range(match_team_df.count()):
        ...
        team_name = match_team_df.collect()[row]['team_name']
        ...
        insert_match_team_sql = f'INSERT INTO match_statistics.match_team(match_team_id, match_id, team_name, final_score, defense_score, attack_score, overtime_score) VALUES (DEFAULT, \'{match_id}\',\'{team_name}\',\'{final_score}\', \'{defense_score}\', \'{attack_score}\', \'{overtime_score}\');'

        conn.execute(text(insert_match_team_sql))
...
def insert_match_player(conn, match_player_df, match_id):
    for row in range(match_player_df.count()):
        ...
        player_name = match_player_df.collect()[row]['player_name']
...
def insert_match_map_team(conn, match_map_team_df, match_id):
    for row in range(match_map_team_df.count()):
        ...
        team_name = match_map_team_df.collect()[row]['team_name']
        map_name = match_map_team_df.collect()[row]['map_name']
        ...
        insert_match_team_sql = f'INSERT INTO match_statistics.match_map_team(match_map_team_id, match_id, map_name, team_name, win, final_score, defense_score, attack_score, overtime_score) VALUES (DEFAULT, \'{match_id}\',\'{map_name}\', \'{team_name}\', \'{win}\', \'{final_score}\', \'{defense_score}\', \'{attack_score}\', \'{overtime_score}\');'
...
def insert_match_map_player(conn, match_map_player_df, match_id):
    for row in range(match_map_player_df.count()):
        ...
        player_name = match_map_player_df.collect()[row]['player_name']
        map_name = match_map_player_df.collect()[row]['map_name']
    ...
    with engine.connect() as conn:
        ...
        for i in range(len(match_data_dict['map'])):
            ...
            if not match_exists(conn, event, match_date, team_one_name, team_two_name):
                ...
                print('---------------------------------------------------------------------------------------')
        print(f'Total Matches Loaded: {match_count}')
        conn.close()
        engine.dispose()
