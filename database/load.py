...
def match_exisits(conn, event, match_date, team_one_id, team_two_id):
    query = f'SELECT event, match_date, team_one_id, team_two_id FROM "match statistics".match WHERE event = \'{event}\' AND match_date = \'{match_date}\' AND team_one_id = {team_one_id} AND team_two_id = {team_two_id};'
...
def get_match_id(conn, event, match_date, team_one_id, team_two_id):
    query = f'SELECT match_id FROM "match statistics".match WHERE event = \'{event}\' AND match_date = \'{match_date}\' AND team_one_id = {team_one_id} AND team_two_id = {team_two_id};'
...
def map_exists(conn, map_name):
    map_query = f'SELECT map_id, map_name FROM "match statistics".map WHERE map_name = \'{map_name}\';'
...
def get_map_id(conn, map_name):
    map_query = f'SELECT map_id FROM "match statistics".map WHERE map_name = \'{map_name}\';'
...
def insert_map(conn, map_name):
    insert_sql = f'INSERT INTO "match statistics".map(map_id, map_name) VALUES (DEFAULT, \'{map_name}\');'
...
def team_exists(conn, team_name):
    team_query = f'SELECT "team_id" FROM "match statistics".team WHERE team_name = \'{team_name}\';'
...
def insert_team(conn,team_name):
    insert_sql = f'INSERT INTO "match statistics".team(team_id, team_name) VALUES (DEFAULT, \'{team_name}\');'
...
def player_exists(conn, player_name):
    player_query = f'SELECT "player_id" FROM "match statistics".player WHERE player_name = \'{player_name}\';'
...
def get_player_id(conn, player_name):
    player_query = f'SELECT player_id FROM "match statistics".team WHERE player_name = \'{player_name}\';'
...
def insert_player(conn,player_name, team_id):
    insert_sql = f'INSERT INTO "match statistics".player(player_id, player_name, team_id) VALUES (DEFAULT, \'{player_name}\' ,\'{team_id}\');'
...
def insert_match_team(conn, match_team_df, match_id):
    for row in range(match_team_df.count()):
        ...
        insert_match_team_sql = f'INSERT INTO "match statistics".match_team(match_team_id, match_id, team_id, final_score, defense_score, attack_score, overtime_score) VALUES (DEFAULT, \'{match_id}\',\'{team_id}\',\'{final_score}\', \'{defense_score}\', \'{attack_score}\', \'{overtime_score}\');'
...
def insert_match_player(conn, match_player_df, match_id):
    for row in range(match_player_df.count()):
        ...
        team_id = get_team_id(conn, match_player_df.collect()[row]['team_name'])
        player_id = get_player_id(conn, match_player_df.collect()[row]['player_name'])


        query = f'INSERT INTO "match statistics".match_player(match_player_id, match_id, player_id, acs_overall, acs_attack, acs_defense, kills_overall, kills_attack, kills_defense, deaths_overall, deaths_attack, deaths_defense, assists_overall, assists_attack, assists_defense, kast_overall, kast_attack, kast_defense, adr_overall, adr_attack, adr_defense, headshot_perecentage_overall, headshot_perecentage_attack, headshot_perecentage_defense, first_kills_overall, first_kills_attack, first_kills_defense, first_deaths_overall, first_deaths_attack, first_deaths_defense)VALUES (DEFAULT, {match_id}, {player_id}, {match_player_df.collect()[row]['acs_overall']}, {match_player_df.collect()[row]['acs_attack']}, {match_player_df.collect()[row]['acs_defense']}, {match_player_df.collect()[row]['kills_overall']}, {match_player_df.collect()[row]['kills_attack']}, {match_player_df.collect()[row]['kills_defense']}, {match_player_df.collect()[row]['deaths_overall']}, {match_player_df.collect()[row]['deaths_attack']}, {match_player_df.collect()[row]['deaths_defense']}, {match_player_df.collect()[row]['assists_overall']}, {match_player_df.collect()[row]['assists_attack']}, {match_player_df.collect()[row]['assists_defense']}, {match_player_df.collect()[row]['kast_overall']}, {match_player_df.collect()[row]['kast_attack']}, {match_player_df.collect()[row]['kast_defense']}, {match_player_df.collect()[row]['adr_overall']}, {match_player_df.collect()[row]['adr_attack']}, {match_player_df.collect()[row]['adr_defense']}, {match_player_df.collect()[row]['headshot_perecentage_overall']}, {match_player_df.collect()[row]['headshot_perecentage_attack']}", "{match_player_df.collect()[row]['headshot_perecentage_defense']}", "{match_player_df.collect()[row]['first_kills_overall']}", "{match_player_df.collect()[row]['first_kills_attack']}", "{match_player_df.collect()[row]['first_kills_defense']}", "{match_player_df.collect()[row]['first_deaths_overall']}", "{match_player_df.collect()[row]['first_deaths_attack']}", "{match_player_df.collect()[row]['first_deaths_defense']}");'
        ...
        conn.commit()
def insert_match_map_team(conn, match_map_team_df):
    pass


def insert_match_map_player(conn, match_map_player_df):
    pass
...
def main():
    ...
    for i in range(len(match_data_dict['map'])):
        ...
        #match_details = match.select("match_date", 'event', 'team_one', 'team_two')
